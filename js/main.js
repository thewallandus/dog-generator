...
fetch(BREEDS_URL)
    ...
    .then(data => {
        ...
        for (let i = 0; i < breedsArray.length; i++) {
            ...
            select.appendChild(option); // adds current option tag to select box list of options
    ...
    const doggoInfo = {
          ...
          return array;
            ...
            assignDislikes() {
                this.dislikes = this.yatesShuffle(this.likesList).slice(0, 2)
                document.getElementById("dislikes").innerHTML = `Dislikes: ${this.likes[0]}, ${this.likes[1]}`
            },
